...
Then /^there should be a part with content type "(.*?)"$/ do |content_type|
  current_email.parts.detect do |p|
    p.content_type.index(content_type)
...
Then /^there should be an email from Ticketee in my box$/ do
  #@mails = @gmail.inbox.find(:unread, from: "sircoolguy99@gmail.com") do |mail|
